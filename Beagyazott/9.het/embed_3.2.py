...
# Define the lower and upper boundaries of a "red" object in HSV color space
red_range = [(0, 100, 100), (10, 255, 255)]

# Initialize the list of tracked points with None values
path = [None] * buffsize
...
while True:
    ...
    if len(cnts) > 0:
        ...
        if radius > 10:
            cv2.circle(frame, (int(x), int(y)), int(radius), (0, 255, 255), -1)  # Fill the circle
    ...
    for i in range(1, len(path)):
        ...
        # Compute thickness based on the distance from the center
        thickness = int(np.sqrt(buffsize / float(i + 1)) * 2.5)
